...
import { createDbClient, guests, invitations, wishlistClaims, wishlistGifts } from '../../node/db';
import { cacheHelper } from '../utils/cacheHelpers'; // helper cache bersih
...
invite.get('/:slug', async (c) => {
    ...
    if (!guest) return c.json({ error: 'Guest not found' }, 403);

    const invitation = await db.select().from(invitations).where(eq(invitations.id, guest.invitationId)).get();
    if (!invitation) return c.json({ error: 'Invitation not found' }, 404);
    if (invitation.status !== 'published') return c.json({ error: 'Invitation not active' }, 403);
    if (invitation.expiresAt && new Date(invitation.expiresAt) < new Date()) return c.json({ error: 'Invitation expired' }, 410);
...
invite.post('/:slug', async (c) => {
    ...
    switch (body.action) {
        ...
        case 'wishlist-claim':
            if (!body.wishlistId) return c.json({ error: 'Wishlist ID required' }, 400);
