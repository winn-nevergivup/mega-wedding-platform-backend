import { and, eq, inArray, InferModel, like, sql } from 'drizzle-orm';
import { Context, Hono } from 'hono';
import { authMiddleware } from 'workers/middleware/auth';
import { applySearchAndPagination } from 'workers/utils/queryHelpers';
import {
    budgetPlanner,
    createDbClient,
    guests,
...
type WishlistGift = InferModel<typeof wishlistGifts>;
type MessageTemplate = InferModel<typeof messageTemplates>;

type TableMap = {
    wishlistGifts: typeof wishlistGifts,
    messageTemplates: typeof messageTemplates,
    budgetPlanner: typeof budgetPlanner,
    weddingChecklist: typeof weddingChecklist,
...
dashboard.get('/user/invitation/:invId', async (c) => {
    ...
    const cacheKey = cacheHelper.getUserKey(
        user.id,
        `inv:${invId}:dashboard:${page}:${limit}:${search}`
    );
    ...
    // Guests
    const guestsList = await applySearchAndPagination(
        db.select().from(guests),
        guests.name,
        search,
        page,
        limit,
        eq(guests.invitationId, invId)
    ).all();

    // Wishlist
    const wishlist = await applySearchAndPagination(
        db.select().from(wishlistGifts),
        wishlistGifts.title,
        search,
        page,
        limit,
        eq(wishlistGifts.invitationId, invId)
    ).all();

    // Wishlist Claims
    const wishlistIds: string[] = wishlist.map((w: WishlistGift) => w.id);
    ...
    // Message templates
    const messages: MessageTemplate[] = await db.select()
        .from(messageTemplates)
        .where(
            search
                ? and(
                    eq(messageTemplates.invitationId, invId),
                    like(messageTemplates.title, `%${search}%`)
                )
                : eq(messageTemplates.invitationId, invId)
        )
        .limit(limit)
...
dashboard.put('/user/invitation/:invId/guest/:guestId/checkin', async (c) => {
    ...
    // Invalidate cache
    // =========================
    await cacheHelper.clearPrefix(
        c.env.CACHE,
        `user:${user.id}:inv:${invId}:guests`
    );

    await cacheHelper.clearPrefix(
        c.env.CACHE,
        `user:${user.id}:inv:${invId}:wishlistClaims`
    );
...
// --------------------
//insert
export async function crudInsert<T extends keyof TableMap>(
    c: Context<{
        Bindings: Bindings;
        Variables: Variables;
    }>,
...
// Update
export async function crudUpdate<T extends keyof TableMap>(
    c: Context<{
        Bindings: Bindings;
        Variables: Variables;
    }>,
    ...
    await db.update(table)
        .set({ ...body, updatedAt: new Date() })
        .where((table as any).id.eq(id)) // TS tidak infer dynamic column → workaround
        .run();
...
// Delete
export async function crudDelete<T extends keyof TableMap>(
    c: Context<{
        Bindings: Bindings;
        Variables: Variables;
    }>,
    ...
    await db.delete(table).where((table as any).id.eq(id)).run();
...
//get
export async function crudGet<T extends keyof TableMap>(
    c: Context<{
        Bindings: Bindings;
        Variables: Variables;
    }>,
    ...
    const cacheKey = cacheHelper.getUserKey(user.id, `inv:${invId}:${(table as any).name}:${page}:${search}`);
    const cached = await cacheHelper.get(c.env.CACHE, cacheKey);
    if (cached) return c.json(JSON.parse(cached));

    const db = createDbClient(c.env.TURSO_DB_URL, c.env.TURSO_DB_AUTH_TOKEN);

    let query: any = db.select().from(table).where((table as any)[invField].eq(invId));
    // Search by 'title' or 'name' if exists
    if (search) {
        if ((table as any).title) query = query.where(like((table as any).title, `%${search}%`));
        else if ((table as any).name) query = query.where(like((table as any).name, `%${search}%`));
    }
...
// GET DETAIL
// ===========================
export async function crudGetDetail<T extends keyof TableMap>(
    c: Context<{
        Bindings: Bindings;
        Variables: Variables;
    }>,
    table: TableMap[T],
    invField?: string // optional, kalau ingin filter by invitation
) {
    const user = c.get('user');
    const id = c.req.param('id');
    ...
    const cacheKey = invId
        ? cacheHelper.getUserKey(user.id, `inv:${invId}:${(table as any).name}:detail:${id}`)
    ...
    let query: any = db.select().from(table).where((table as any).id.eq(id));

    // filter by invitationId kalau perlu
    if (invField && invId) {
        query = query.where((table as any)[invField].eq(invId));
    }
...
export async function crudGetWishlistClaims(c: Context<{
    Bindings: Bindings;
    Variables: Variables;
}>) {
    const user = c.get('user');
    ...
    const cacheKey = cacheHelper.getUserKey(
        user.id,
        `inv:${invId}:wishlistClaims:${page}:${search}`
    );
    ...
    if (search) {
        ...
        claimsQuery = claimsQuery.where(
            inArray(wishlistClaims.guestId, matchingGuestIds)
        );
    }
