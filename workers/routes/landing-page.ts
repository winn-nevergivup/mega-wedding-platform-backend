...
import { articles, createDbClient, partners, ratings, themes } from '../../node/db';
...
// ===========================
async function crudGet(c: Context<{
    Bindings: Bindings;
    Variables: Variables;
}>, table: any, cachePrefix: string, searchField = 'name', extraFilter?: any) {
    ...
    if (edgeCached) return c.json(edgeCached);
    ...
    if (extraFilter) query = query.where(extraFilter);
    if (search) query = query.where(like((table as any)[searchField], `%${search}%`));
...
async function crudCreate(c: Context<{
    Bindings: Bindings;
    Variables: Variables;
}>, table: any, cachePrefix: string, extraValues = {}) {
    const db = createDbClient(c.env.TURSO_DB_URL, c.env.TURSO_DB_AUTH_TOKEN);
...
async function crudUpdate(c: Context<{
    Bindings: Bindings;
    Variables: Variables;
}>, table: any, cachePrefix: string) {
    const db = createDbClient(c.env.TURSO_DB_URL, c.env.TURSO_DB_AUTH_TOKEN);
    const id = c.req.param('id');
    const body = await c.req.json();

    await db.update(table).set({ ...body, updatedAt: new Date() }).where(eq((table as any).id, id)).run();
...
async function crudDelete(c: Context<{
    Bindings: Bindings;
    Variables: Variables;
}>, table: any, cachePrefix: string) {
    const db = createDbClient(c.env.TURSO_DB_URL, c.env.TURSO_DB_AUTH_TOKEN);
    ...
    await db.delete(table).where(eq((table as any).id, id)).run();
