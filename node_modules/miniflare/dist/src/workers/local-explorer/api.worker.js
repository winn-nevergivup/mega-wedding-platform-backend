...
var tryDecode = (str, decoder) => {
  ...
  for (; i < url.length; i++) {
    ...
    if (charCode === 37) {
      let queryIndex = url.indexOf("?", i), path = url.slice(start, queryIndex === -1 ? void 0 : queryIndex);
...
// ../../node_modules/.pnpm/hono@4.11.5/node_modules/hono/dist/router/pattern-router/router.js
...
// ../../node_modules/.pnpm/hono@4.11.5/node_modules/hono/dist/preset/tiny.js
  ...
  switch (target) {
    ...
    case "form": {
      ...
      formData.forEach((value2, key) => {
        key.endsWith("[]") ? (form[key] ??= []).push(value2) : Array.isArray(form[key]) ? form[key].push(value2) : key in form ? form[key] = [form[key], value2] : form[key] = value2;
      }), value = form;
      break;
    }
    case "query":
...
var zD1QueryMeta = external_exports.object({
  ...
  query: external_exports.object({
    ...
    per_page: external_exports.number().gte(1).lte(1e3).optional().default(20),
...
), zCloudflareD1ListDatabasesData = external_exports.object({
  body: external_exports.never().optional(),
  path: external_exports.never().optional(),
  query: external_exports.object({
    name: external_exports.string().optional(),
    page: external_exports.number().gte(1).optional().default(1),
    per_page: external_exports.number().gte(10).lte(1e4).optional().default(1e3)
  }).optional()
}), zCloudflareD1ListDatabasesResponse = zD1ApiResponseCommon.and(
  external_exports.object({
...
), zCloudflareD1RawDatabaseQueryData = external_exports.object({
  body: zD1BatchQuery,
  path: external_exports.object({
    database_id: zD1DatabaseIdentifier
  }),
  query: external_exports.never().optional()
}), zCloudflareD1RawDatabaseQueryResponse = zD1ApiResponseCommon.and(
  external_exports.object({
...
async function listD1Databases(c, query) {
  let { page, per_page, name } = query, d1BindingMap = c.env.LOCAL_EXPLORER_BINDING_MAP.d1, databases = Object.entries(d1BindingMap).map(([id, bindingName]) => ({
    name: bindingName.split(":").pop() || bindingName,
    uuid: id,
    version: "production"
  })), totalCount = databases.length;
  name && (databases = databases.filter(
    (db) => db.name?.toLowerCase().includes(name.toLowerCase())
  ));
  let startIndex = (page - 1) * per_page, endIndex = startIndex + per_page;
  return databases = databases.slice(startIndex, endIndex), c.json({
...
async function rawD1Database(c, body) {
  let databaseId = c.req.param("database_id"), db = getD1Binding(c.env, databaseId);
  if (!db)
    return errorResponse(404, 1e4, "Database not found");
...
async function listKVNamespaces(c, query) {
  let direction = query.direction ?? "asc", order = query.order ?? "id", page = query.page, per_page = query.per_page, kvBindingMap = c.env.LOCAL_EXPLORER_BINDING_MAP.kv, namespaces = Object.entries(kvBindingMap).map(([id, bindingName]) => ({
    id,
  ...
  let total_count = namespaces.length, startIndex = (page - 1) * per_page, endIndex = startIndex + per_page;
  return namespaces = namespaces.slice(startIndex, endIndex), c.json({
...
async function listKVKeys(c, query) {
  let namespace_id = c.req.param("namespace_id"), cursor = query.cursor, limit = query.limit, kv = getKVBinding(c.env, namespace_id);
  if (!kv)
    return errorResponse(404, 1e4, "Namespace not found");
  let listResult = await kv.list({ cursor, limit }), resultCursor = "cursor" in listResult ? listResult.cursor ?? "" : "";
  return c.json({
    ...wrapResponse(
...
async function getKVValue(c) {
  let namespace_id = c.req.param("namespace_id"), key_name = c.req.param("key_name"), kv = getKVBinding(c.env, namespace_id);
  if (!kv)
    return errorResponse(404, 1e4, "Namespace not found");
  let value = await kv.get(key_name, { type: "arrayBuffer" });
  return value === null ? errorResponse(404, 1e4, "Key not found") : new Response(value);
}
...
async function deleteKVValue(c) {
  let namespace_id = c.req.param("namespace_id"), key_name = c.req.param("key_name"), kv = getKVBinding(c.env, namespace_id);
  return kv ? (await kv.delete(key_name), c.json(wrapResponse({}))) : errorResponse(404, 1e4, "Namespace not found");
}
async function bulkGetKVValues(c, body) {
  let namespace_id = c.req.param("namespace_id"), { keys } = body, kv = getKVBinding(c.env, namespace_id);
  if (!kv)
    return errorResponse(404, 1e4, "Namespace not found");
...
// src/workers/local-explorer/api.worker.ts
var BASE_PATH = "/cdn-cgi/explorer/api", app = new Hono2().basePath(BASE_PATH);
...
var api_worker_default = app;
export {
  api_worker_default as default
};
